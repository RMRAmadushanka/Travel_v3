...
const CommentListing: FC<CommentListingProps> = ({ className = "", data, hasListingTitle, fetchUpdatedFeedbacks }) => {
  ...
  const handleUpdateComment = async () => {
    ...
    try {
      const response = await fetch("/api/car-feedback/update-comment", {
        method: "PATCH",
  ...
  const handleDeleteComment = async () => {
    ...
    try {
      const response = await fetch(`/api/car-feedback/${data._id}`, {
        method: "DELETE",
      });
        ...
        <Modal title="Verify Email" onClose={() => setIsEmailPopupOpen(false)}>
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting, setFieldError }) => {
              try {
                const response = await fetch("/api/car-feedback/verify-email", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ email: values.email, carId, userId }),
                });
